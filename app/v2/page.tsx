...
import { Code, Palette, Zap, Search, Server, Users, ArrowRight, ExternalLink, ChevronLeft, ChevronRight, Github, Linkedin, Mail, Globe, Coffee, ChevronUp, ChevronDown, MessageSquare, PenTool, Code2, CheckCircle } from 'lucide-react'
      ...
      <section id="services" className="py-20 sm:py-16 relative z-10 bg-black text-white">
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <Tabs defaultValue="design" className="w-full">
            <TabsList className="grid bg-transparent grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-4 mb-8">
              {services.map((service) => (
                <TabsTrigger
                  key={service.id}
                  value={service.id}
                  className="flex flex-col items-center text-center p-2 data-[state=active]:bg-white rounded-lg text-xs sm:text-sm"
                >
                  {service.icon}
                  <span className="mt-1 sm:mt-2">{service.title}</span>
                </TabsTrigger>
              ))}
            </TabsList>
            {services.map((service) => (
              <TabsContent key={service.id} value={service.id} className="mt-44 sm:mt-32 lg:mt-0">
